    ...
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (req.body.author.equals(req.user._id)) {
          ...
          err = new Error(`${req.user_.id} is not the author of this comment!`);
          err.status = 403;
          return next(err);
        }
      })
  ...
  .delete(
    ...
    (req, res, next) => {
      Campsite.findById(req.params.campsiteId)
        .then((campsite) => {
          if (campsite && campsite.comments.id(req.params.commentId)) {
            //checking if ID is the same as the owner of the comment//
            if (
              req.user._id.equals(
              ...
              err = new Error(
                `${req.user_.id} is not the author of this comment!`
              );
              err.status = 403;
              return next(err);
            }
